    ...
    >
      <ScrollArea h="calc(100vh - 220px)" offsetScrollbars type="never">
        {items.length === 0 ? (
          ...
          <Stack gap="sm">
            {items.map((item) => (
              <Paper key={item.id} withBorder p="sm" radius="md" shadow="xs">
                <Group justify="space-between" align="center" wrap="nowrap">
                  ...
                  <ActionIcon color="red" variant="light" size="lg" onClick={() => onRemove(item.id)} aria-label={`Remove ${item.title}`}>
                    <IconTrash size={18} />
                  </ActionIcon>
                </Group>
              </Paper>
      ...
      {items.length > 0 && (
        ...
        >
          ...
          <Group grow>
            ...
            <Button leftSection={<IconCreditCard size={18} />} color="teal" onClick={onCheckout} size="md" radius="md">
              Checkout Now
            </Button>
          </Group>
        </Box>
